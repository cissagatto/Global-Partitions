...
run.ecc.utiml <- function(ds, 
  ...
  cat("\n##############################################################\n\n")
  time.execute = system.time(execute.global.utiml(ds, 
                                                   dataset_name, 
                                                   number_folds, 
                                                   number_cores, 
                                                   folderResults))
  ...
  cat("\n##############################################################\n\n")
  time.evaluate = system.time(evaluate.global.utiml(ds, 
                                                     dataset_name, 
                                                     number_folds, 
                                                     number_cores, 
                                                     folderResults))
  ...
  cat("\n##############################################################\n\n")
  time.gather = system.time(gather.eval.utiml(ds, 
                                                dataset_name, 
                                                number_folds, 
                                                number_cores, 
                                                folderResults))
  ...
  cat("\n# RUN: Save Runtime                                          #")
  cat("\n##############################################################\n\n")
  RunTimeGlobal = rbind(time.execute, time.evaluate, time.gather)
  setwd(diretorios$folderGlobal)
  write.csv(RunTimeGlobal, paste(dataset_name, "-RunTime-Utiml.csv", sep=""))
